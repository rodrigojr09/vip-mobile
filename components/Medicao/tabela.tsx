...
import { calculateCalor, calculateIBUTG, calculateMediaIBUTG } from "./calculo";
		...
		<View>
			<View style={styles.row}>
				{tableHead.map((header, index) => (
					<Text key={index as any} style={[styles.cell, styles.headerCell]}>
						{header}
					</Text>
				))}
			</View>
			...
			{tableData.map((rowData, rowIndex) => (
				<View key={rowIndex as any} style={styles.row}>
					{rowData.map((cellData: any, cellIndex: number) => (
						<TextInput
							key={cellIndex as any}
							style={[styles.cell, styles.input]}
							value={cellData}
							editable={cellIndex !== 4}
							onChangeText={(text) =>
								handleInputChange(text, rowIndex, cellIndex)
							}
							placeholder="Digite..."
						/>
					))}
