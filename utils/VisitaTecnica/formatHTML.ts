...
function getRespostaHtml(resposta: VIPVisitaType["respostas"][number]): string {
	return `
      <div class="card">
        <h3 class="font-bold">${resposta.pergunta}</h3>
        <p class="answer">Resposta: ${resposta.checked ? "Sim" : "Não"}</p>
        ${
    ...
    <div class="p-2 h-screen">
      <div class="border border-black p-2 h-full">
        ...
        <div class="max-w-[90vw] mb-[20vh] break-words mx-auto">
          ...
          <p>Confirmo que fui designado como responsável por informar todos os dados necessários para a elaboração da documentação de Saúde e Segurança do Trabalho da empresa "$empresa_nome".</p>
  ...
  <head>
    ...
    <style>
      * {
      ...
      .h-screen { min-height: 100vh; page-break-after: always; }
      .p-2 { padding: 0.5rem; }
  ...
  <body>
    <div class="container">
      ...
      <div class="info">
        ...
        <p><strong>Técnico Responsável pela Visita:</strong> ${
			visita.tecnico
		}</p>
        <p><strong>Responsável pela Empresa:</strong> ${visita.responsavel}</p>
        <p><strong>Data:</strong> ${visita.data}</p>
      </div>
  
      ${visita.respostas.map(getRespostaHtml).join("")}
