...
export default function Config() {
	...
	useEffect(() => {
		(async () => {
			try {
				const data = JSON.parse((await storage.getLevantamentos()) || "[]").map(
					(lev: { empresa: VIPEmpresaType }) => ({
						id: lev.empresa.id || "-",
	...
	const apagarLevantamentos = async () => {
		Alert.alert(
			...
			[
				{ text: "Cancelar", style: "cancel" },
				{
					...
					onPress: async () => {
						try {
							await storage.clearLevantamentos(); // limpa os dados salvos
							setLevantamentos([]);
								...
								Alert.alert("ID do Levantamento", item.id, [
									{
										...
										style: "default",
										onPress: async () => {
											const empresa = await storage.getLevantamento(item.id);
											if (!empresa) {
												Alert.alert(
													"Erro",
